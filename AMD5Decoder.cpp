...
AMD5Decoder::AMD5Decoder(const char *fname) : AMeshDecoder(fname)
{
  init();
  openFile(fname);
  if(!err.getError()) readObjects(); else return;
  if(!err.getError()) convertAll(); else return;
}
