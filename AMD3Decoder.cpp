...
AMD3Decoder::AMD3Decoder(const char *fname) : AMeshDecoder(fname)
{
  ...
  if(!err.getError()) readBFrames(); else return;
  if(!err.getError()) readTags(); else return;
  if(!err.getError()) readMeshes(); else return;
}
...
void AMD3Decoder::readMeshHeader()
{
  ...
  if(strncmp("IDP3",(char *)magic,4)) {
    //DEBUG_OUT<<"Bad mesh magic number, giving up!\n");
    //errorFlag=true;
    return;
  }
  char meshName[64];
...
void AMD3Decoder::readTextures()
{
  ...
  for(unsigned int i=0;i<ntextures;i++) {
    ...
    index=readLongL();
    actualName[0]=0;
    DSHELPER_alib.fillInNameFromFullName(actualName,name);
