...
AMD2Decoder::AMD2Decoder(const char *fname) : AMeshDecoder(fname)
{
  ...
  if(!err.getError()) readTriangles(); else return;
  if(!err.getError()) {
    if(ntextures) readSkins();
    else {
      char *maybeName=strdup(fname);
      unsigned long sl=strlen(maybeName);
      maybeName[sl-3]='b';
      maybeName[sl-2]='m';
      maybeName[sl-1]='p';
      DEBUG_OUT<<"Model has no skins/textures, trying '"<<maybeName<<"'...\n";
...
void AMD2Decoder::readFrames()
{
  ...
  for(unsigned int i=0;i<nframes;i++) {
    ...
    for(unsigned int j=0;j<nverts;j++) {
      ...
      //temp=theVert->x; theVert->x=theVert->z;  theVert->z=temp;
...
void AMD2Decoder::readTriangles()
{
  ...
  DEBUG_OUT<<"There are "<<nfaces<<" triangles (with matching faceTextCoords).\n";
  if(!nfaces) return;
...
void AMD2Decoder::readSkins()
{
  ...
  for(unsigned i=0;i<ntextures;i++) {
    ...
    actualName[0]=0;
    DSHELPER_alib.fillInString(actualName,pathPartOfOriginalName);
    if(strlen(actualName)) DSHELPER_alib.fillInString(actualName,"/");
    DSHELPER_alib.fillInNameFromFullName(actualName,skinName);
    DEBUG_OUT<<"skin "<<i<<": "<<skinName<<" (using "<<actualName<<")\n";
    theSkin=new ATexture3D(actualName);
    theSkin->getBitmap()->reorderRGBChunky2BGR();
