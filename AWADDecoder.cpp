...
float AWADDecoder::wad2FloatX(int16_t x)
{
  float r=0.0;
  if(x&0x8000) x=-x;
  if(x) r=((float)x)/32768.0F;
  return r;
}


float AWADDecoder::wad2FloatY(int16_t y)
{
  float r=0.0;
  if(y&0x8000) y=-y;
  if(y) r=-((float)y)/32768.0F;
  return r;
}
...
void AWADDecoder::readVertices(unsigned int entry)
{
  ...
  for(unsigned int t=0;t<numVertices;t++) {
    ...
    vi->x=sx;  vi->y=sy;
    vf->x=wad2FloatX(vi->x);
    vf->y=wad2FloatY(vi->y);
...
void AWADDecoder::readThings(unsigned int entry)
{
  ...
  for(unsigned int t=0;t<numThings;t++) {
    ...
    if(th->type==1) {
      ...
      playerX=wad2FloatX(th->x);  playerY=wad2FloatY(th->y);  playerZ=0.0;
      //playerAngle=th->angle;
      playerXAngle=90.0;  playerYAngle=0.0;  playerZAngle=0.0;
    }
  }
}
