...
AMDLDecoder::AMDLDecoder(const char *fname) : AMeshDecoder(fname)
{
  init();
  openFile(fname);
  if(!err.getError()) readHeader(); else return;
  if(!err.getError()) readTextures(); else return;
  if(!err.getError()) readTextCoords(); else return;
  if(!err.getError())  {
    readTriangles();
    buildAndFixFaceAndTextCoordLists();
  } else return;
  if(!err.getError()) readFrames(); else return;
}
...
void AMDLDecoder::readHeader()
{
  ...
  if(vers!=6) {
    //DEBUG_OUT<<"Bad version number, giving up!\n");
    //errorFlag=true;
    return;
  }
  ...
  if((synctype!=0)&&(synctype!=1)) {
    //DEBUG_OUT<<"synctype field has bad value, giving up!\n");
    //errorFlag=true;
    return;
  }
  flags=readLongL();
...
void AMDLDecoder::readTextures()
{
  ...
  for(unsigned int t=0;t<ntextures;t++) {
    ...
    if((group!=0)&&(group!=1)) {
      DEBUG_OUT<<"\n";
      std::cerr<<"Group field has bad value, giving up!\n";
      //errorFlag=true;
      return;
    }
    if(group) {
      ...
      DEBUG_OUT<<" group of "<<nb<<"\n";
      // error out here, we don't understand this format fully
      //DEBUG_OUT<<"We don't understand the format!\n");
      //errorFlag=true;
      std::cerr<<"group==1, not implememted, giving up!\n";
      return;
    /*
      for(i=0;i<nb;i++) {
        time=readFloatL();
    ...
    */
      ...
      uint8_t *ptr=(uint8_t *)theText->getBitmap()->getPlane(0);
      fread(ptr,skinwidth*skinheight,1,fd);
      APalette *theColors=(APalette *)theText->getBitmap()->getColors();
      for(unsigned int j=0;j<256;j++) {
        theColors->setColor(j,AQuakePalette[j][0],AQuakePalette[j][1],AQuakePalette[j][2]);
      }
      if(!theText->getBitmap()->isTwentyFourBitsChunky()) {
        //std::cerr<<"ABitmap::convert() not implemented!\n";
        //exit(EXIT_FAILURE);
        theText->getBitmap()->convertBitmap(24,ALIB_BITMAP_CHUNKY,NULL);
        theText->getBitmap()->reorderRGBChunky2BGR();
      }
...
void AMDLDecoder::readFrames()
{
  ...
  for(unsigned int i=0;i<nframes;i++) {
    ...
    if((type!=0)&&(type!=1)) {
      DEBUG_OUT<<"\n";
      //DEBUG_OUT<<"type field has bad value, giving up!\n");
      //errorFlag=true;
      return;
    }
    if(type) {
