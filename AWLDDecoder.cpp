...
AWLDDecoder::AWLDDecoder(const char *fname) : ASceneDecoder(fname)
{
  ...
  openFile(fname);
  curSurfMap=NULL;
  if(!err.getError()) readObjectsFromFile(fd);
}
...
void AWLDDecoder::readObjectsFromFile(FILE *fd)
{
  ...
  while(reading) {
    ...
    fgets(line,256,fd);
    if(!feof(fd)) {
      DSHELPER_alib.parseString(line,NULL,false,',');
      //
    }
    switch(line[0]) {
      case '#':  // comment
      ...
      case 's':  // stepsize or surface def/map/etc?
        if(!strncmp(line,"surfacedef",10)) {
          sscanf(line,"%s %s %lx",theCmd,theName,&x);
        ...
        if(!strncmp(line,"surface ",8)) {
         sscanf(line,"%s %lx %s",theCmd,&x,theName);
      ...
      case 'p':  // polyobj or polyobj2?
        ...
        if(!strncmp(line,"polyobj",7)) {
          ...
          sscanf(line,"%s %ld %s %f,%f,%f %f,%f,%f %f,%f,%f %f,%f,%f",theCmd,&n,misc1,&ax,&ay,&az,&bx,&by,&bz,&cx,&cy,&cz,&dx,&dy,&dz);
          ...
          theFaceInfo->surf=1;  // for now
          lookupRend386Color(theFaceInfo->surf,&theFaceInfo->fRed,&theFaceInfo->fGreen,&theFaceInfo->fBlue);
          theFace=new AFace3D;
          theMesh=new AMesh3D(theFace,theFaceInfo,vd,vc,vb,va);
      ...
      case 'l':  // light?
        ...
        sscanf(line,"%s %ld,%ld,%ld",theCmd,&x,&y,&z);
      ...
      case 'c':  // camera?
        ...
        sscanf(line,"%s %ld %f,%f,%f %f,%f,%f",theCmd,&n,&fx,&fy,&fz,&fa,&fb,&fc);
        // Because people are sloppy and sometimes use commas and sometimes spaces...
        if((fy==0.0)&&(fz==0.0)&&(fb==0.0)&&(fc==0.0)) sscanf(line,"%s %ld %f %f %f %f %f %f",theCmd,&n,&fx,&fy,&fz,&fa,&fb,&fc);
      ...
      case 'o':  // object? option?
        if(!strncmp(line,"object",6)) {
          ...
          finalName=DSHELPER_alib.toLower(actualName);
...
void AWLDDecoder::fixObjectSurfMaps(AWLDDecoderObject *theObj)
{
  ...
  unsigned long nfaces=theObj->mesh->getNumFaces();
...
unsigned long AWLDDecoder::lookupSurfNum(char *surfMapName,unsigned long surfNum)
{
  if(!surfMapName) return 0;
  unsigned long res=0;
  AWLDDecoderSurfaceMap *cmap=NULL;
  AWLDDecoderSurfaceMap *map=NULL;
  surfaceMaps.jumpToHead();
  bool looking=true;
  while(looking) {
    cmap=(AWLDDecoderSurfaceMap *)surfaceMaps.info();
  ...
  if(map) {
    ...
    map->surfaces.jumpToHead();
    looking=true;
    while(looking) {
      csurf=(AWLDDecoderSurface *)map->surfaces.info();
    ...
    if(surf) {
      ...
      surfaceDefs.jumpToHead();
      looking=true;
      while(looking) {
        csurfdef=(AWLDDecoderSurfaceDef *)surfaceDefs.info();
      ...
      //else DEBUG_OUT<<"lookupSurfNum didn't find that surfdef!\n");
    }
    //else DEBUG_OUT<<"lookupSurfNum didn't find that surf!\n");
  }
  //else DEBUG_OUT<<"lookupSurfNum didn't find that map!\n");
  return res;
}
